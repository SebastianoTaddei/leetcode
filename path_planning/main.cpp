...
Path astar(Grid const &grid, Obstacles const &obstacles, Point const &start, Point const &goal)
{
  ...
  while (not queue.empty())
  {
    ...
    auto const cost = dist.at(row).at(col);
    for (auto const &move : moves)
    {
      auto const next_row = row + move.first;
      auto const next_col = col + move.second;
      ...
      auto const new_cost = cost + grid.at(next_row).at(next_col);
      if (new_cost < dist.at(next_row).at(next_col) or came_from.find(next_point) == came_from.end())
      {
        ...
        queue.emplace(new_cost + heuristic, next_point);
        dist.at(next_row).at(next_col) = new_cost;
      }
    }
  }
...
int path_cost(Grid const &grid, Path const &path)
{
  int cost{0};
  for (auto const &point : path)
  {
    cost += grid.at(point.first).at(point.second);
  }
...
int main()
{
  ...
  { // BFS
    ...
    std::cout << "Breadth First Search: " << path_cost(grid, path) << '\n';
    print_solution(grid, path, obstacles);
  }
  { // DFS
    auto const path = dfs(grid, obstacles, start, goal);
    std::cout << "Depth First Search: " << path_cost(grid, path) << '\n';
    print_solution(grid, path, obstacles);
  }
  { // Dijkstra
    auto const path = dijkstra(grid, obstacles, start, goal);
    std::cout << "Dijkstra: " << path_cost(grid, path) << '\n';
    print_solution(grid, path, obstacles);
  }
  { // Greedy
    auto const path = greedy(grid, obstacles, start, goal);
    std::cout << "Greedy: " << path_cost(grid, path) << '\n';
    print_solution(grid, path, obstacles);
  }
  { // A*
    auto const path = astar(grid, obstacles, start, goal);
    std::cout << "A*: " << path_cost(grid, path) << '\n';
    print_solution(grid, path, obstacles);
  }
}
