...
[[nodiscard]] int find_minimum(std::vector<int> const &nums)
{
  int left  = 0;
  ...
  while (left < right)
  {
    ...
    auto const bound  = nums.at(right);
    auto const value  = nums.at(middle);
...
{
  { // Example 1
    std::vector<int> const nums = {1, 3, 5};
    std::cout << "Example 1: " << find_minimum(nums) << '\n';
  }
  { // Example 2
    std::vector<int> const nums = {2, 2, 2, 0, 1};
    std::cout << "Example 2: " << find_minimum(nums) << '\n';
  }
}
