...
1, you can move to any of the cells (x + 1, 0), (x + 1, 1), ..., (x + 1, n - 1).
Note that it is not possible to move from cells in the last row.

Each possible move has a cost given by a 0-indexed 2D array moveCost of size (m
* n) x n, where moveCost[i][j] is the cost of moving from a cell with value i to
a cell in column j of the next row. The cost of moving from cells in the last
...
[[9,8],[1,5],[10,12],[18,6],[2,4],[14,3]] Output: 17 Explanation: The path with
the minimum possible cost is the path 5 -> 0 -> 1.
...
[[12,10,15],[20,23,8],[21,7,1],[8,1,13],[9,10,25],[5,3,2]] Output: 6
...
int minimum_path_cost(std::vector<std::vector<int>> const &grid, std::vector<std::vector<int>> const &move_cost)
{
  ...
  for (int row{1}; row < rows; row++)
  {
    for (int col{0}; col < cols; col++)
    {
      ...
      {
        auto const
          new_cost = dp.at(row - 1).at(i) + move_cost.at(grid.at(row - 1).at(i)).at(col)
                     + grid.at(row).at(col);
        min_cost = std::min(min_cost, new_cost);
      }
      dp.at(row).at(col) = min_cost;
    }
  }

  int min_cost       = INT_MAX;
  auto const &dp_end = dp.back();
  for (int col{0}; col < cols; col++)
  {
    min_cost = std::min(min_cost, dp_end.at(col));
  }
...
int main()
{
  { // Example 1
    ...
    std::vector<std::vector<int>> move_cost = {{9, 8}, {1, 5}, {10, 12}, {18, 6}, {2, 4}, {14, 3}};
    std::cout << "Example 1: " << minimum_path_cost(grid, move_cost) << '\n';
  }
  { // Example 2
    ...
    std::cout << "Example 2: " << minimum_path_cost(grid, move_cost) << '\n';
  }
}
